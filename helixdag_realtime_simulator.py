        ...
        while not self.stop_event.is_set() and not self.global_state['stop_all']:
            ...
            while not found and not self.stop_event.is_set() and not self.global_state['stop_all']:
                ...
                if hash_meets_target(digest, difficulty):
                    ...
                    # broadcast to network (global_state)
                    ...
                    if self.stats.last_block_time is not None:
                        ...
                        # EWMA update only in CMDA mode
                        if self.mode == 'cmda':
                            self.stats.ewma_interval = (1 - ALPHA) * self.stats.ewma_interval + ALPHA * observed_interval
                            # adjust local difficulty: additive proportional rule
                            # keep difficulty >= 2
                            adjustment = ALPHA * (TARGET_INTERVAL - self.stats.ewma_interval)
                            # multiplicative scaling to keep integer difficulty
                            new_diff = max(2, int(self.stats.local_difficulty * (1.0 - adjustment / max(0.001, TARGET_INTERVAL))))
                            self.stats.local_difficulty = new_diff
